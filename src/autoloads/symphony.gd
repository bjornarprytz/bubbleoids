...
func _process(delta: float) -> void:
    ...
    var current_playback_position = current_player.get_playback_position()

    if current_playback_position < prev_playback_position:
        # Looping back to start of track
        _restart_beat(current_playback_position)
...
func _restart_beat(drift_calibration: float):
    time_counter = drift_calibration
    beat_number = 0
    # No need to .play() because the audio files are set to loop already
...
func switch_to_track(number: int):
    ...
    if current_player.stream != null:
        ...
        fade_out_tween.tween_property(current_player, "volume_db", -80.0, .69)
        fade_out_tween.tween_callback(current_player.stop)
...
func pop_instrument() -> bool:
    if current_track_index <= 0:
        ...
        var current_player = get_current_player()
        var fade_out_tween = create_tween()
        fade_out_tween.tween_property(current_player, "volume_db", -80.0, .19)
        fade_out_tween.tween_callback(current_player.stop)
        return false
